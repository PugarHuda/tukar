...
type Config struct {
	...
	BridgeToken   string // bearer token the Next.js app must present on POST /trisa/transfer
...
func LoadConfig() (*Config, error) {
	c := &Config{
		...
		BridgeToken:   os.Getenv("TRISA_BRIDGE_TOKEN"),
	...
	if len(c.BridgeToken) < 16 {
		return nil, fmt.Errorf("TRISA_BRIDGE_TOKEN is required (>= 16 chars; the same value the webapp sends as its bearer token)")
	}
	return c, nil
}
