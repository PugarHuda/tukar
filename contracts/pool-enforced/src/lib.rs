...
//! Tukar pool-enforced — a PARALLEL, preview-track copy of the live corridor pool
//! that additionally enforces per-corridor amount caps ON-CHAIN by reading the live
//! policy-registry cross-contract in `withdraw`. Identical to `contracts/pool/` except
//! for: `DataKey::PolicyRegistry` + `set_policy_registry`/`policy_registry`, the
