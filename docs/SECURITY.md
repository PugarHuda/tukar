...
not use it with real assets. Every property below is exercised, not asserted.

## Disclosed key compromise (found 2026-09-11)

The corridor admin secret (`corredor`, public key
`GB2CVRVNR4VN5LYVOX637ZS46RJONKWVQZ4IZC5IIEPAPPFRC5CHYRVS`) was hardcoded in four
tracked scripts and pushed to a public GitHub remote. It entered the repository in
commit `a6b44a0` and spread to `409ad22`, `adb4ef6` and `46ee560`. It was found on
2026-09-11 and removed from the working tree in `a647609`; those scripts now read it
from the environment. Removal from the tree does not remove it from history, so the key
is treated as compromised.

This is a testnet key and no real money can be taken with it. A holder gets the eight
admin calls on the live pool: the ASP root, the deny list, the auditor, the FX oracle,
the three disclosure verifier setters, and audit-request registration. The four core
verifiers (transfer, compliance, disclosure, merkleUpdate) are written once at
construction and have no setter, so the proofs that move money cannot be repointed;
there is no admin withdraw, no mint and no pause. The live pool has neither `upgrade`
nor `set_admin`, so its admin cannot be rotated in place. The fix is migrating the live
corridor onto the upgradeable pool under a fresh key, which is deliverable D1.3 of the
SCF build proposal. Full analysis in `docs/THREAT_MODEL.md` section 3.5.
...
| **Pool funds (USDC)** | The pool releases tokens only for a valid, *bound* withdraw. It never releases more than a proof authorizes, and never pays out on a replay of a spent note. |
...
| T9 | **Lie in a disclosure** (claim a false amount) | Groth16 soundness means a false witness can't be proven, and a tampered public input fails verification | rejected off-chain **and** on-chain (`InvalidProof`), live |
...
- **Anchors.** Fiat on/off-ramps run real SEP-10/24 against SDF's testnet reference
  ...
  pending for mainnet.
- **Phase-2 of the setup** is now a **multi-party ceremony** (3 contributions + a
  ...
  guarantee needs genuinely independent parties.
...
sound**, with no critical or high break:
...
- *Mod-r agreement:* the browser's `keccak % r` equals the contract's
  `Bn254Fr::from_bytes` because the SDK reduces mod r, which is true on soroban-sdk
...
- **Re-broadcast race** (low): an observer can copy a pending withdraw and submit it
  with the *same* recipient; the loser just gets `NullifierUsed`, with **no fund
  loss** because the recipient is bound and funds still reach the intended party. A per-tx
...
- **Privacy is therefore *unlinkability of the graph*, and its strength = the
  anonymity set**, which is the number of concurrent same-amount notes a deposit
  could correspond to. A passive observer who sees a deposit of amount X and a
  withdrawal of amount X can link them *unless* many other in-flight notes share
  that amount.
...
- **The shared demo key collapses it to zero.** One key on both edges self-links
  every payment (see the demo-key caveat above).
...
bucket), and Tukar's 2-in/2-out `transfer` would need several sequential splits,
which is a real circuit/UX redesign rather than a config flip. So for
arbitrary-amount remittance, link-privacy is fundamentally an **operational**
property, and the honest levers are: (1) a large
...
the exact deposited amount, by using the JoinSplit re-denomination that already exists
...
- **Contract unit tests** (55/55): `cd contracts/pool && cargo test`. Covers
