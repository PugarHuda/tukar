...
| **Transfer input amounts not range-checked (latent / defense-in-depth)** — `transfer.circom` range-checks outputs to 248 bits but not inputs; value conservation is sound *today* only because every tree leaf is inductively ≤2²⁴⁸ (deposits 64-bit, outputs 248-bit), so a 2-input sum can't wrap the field. | low/latent | **Known** — not currently exploitable; the proper hardening is a `Num2Bits(248)` on each `inAmount` (needs a transfer-circuit + verifier redeploy — deferred to avoid touching the largest circuit's trusted setup pre-judging). |
...
- **Contract unit tests** (19/19): `cd contracts/pool && cargo test` — covers
