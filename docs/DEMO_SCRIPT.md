...
> - **Automated (`npm run video`).** Nothing is recorded by hand. A natural neural
>   voiceover (`edge-tts`, per scene) + a Playwright recording of the **real live
>   on-chain run** (landing → connect → deposit → off-ramp → withdraw → disclosure →
>   tamper) + an ffmpeg mux that places each VO clip at its scene start →
>   `build-video/tukar-narrated.mp4`.
...
>   the cut to submit. (`build-video/` is gitignored, so upload the mp4 and paste the
>   link into the README's *Demo video* slot.) The VO lines live in `scripts/make-vo.py`.
> - **Manual.** Record the live site yourself and narrate with the shot-by-shot script
>   below. There are two cuts: the **2:00 fast cut** (next section) for a tight judge
>   video, and the **full 2.5-3 min walkthrough** (after the divider) with every beat +
>   optional scenes. Same live site, same beats. The fast cut just trims the narration.
...
> "On the public ledger you see only a commitment. The amount and the recipient are
> shielded. None of this is a mock. That count is read live from the pool, real USDC
> just entered custody, and the Merkle tree advanced trustlessly with a proof. No
> admin can forge a root."

**1:00-1:25 · Off-ramp**: *Receiver panel: Reveal & off-ramp → shows ≈ +MXN*
> "It arrives still shielded. The amount is revealed only at the off-ramp edge.
> 500 USDC becomes about 8,700 pesos, at a rate the pool reads on-chain from
> Reflector, Stellar's FX oracle, instead of a hardcoded number. And that oracle is
> load-bearing, because a withdrawal won't settle below a live floor."
...
> trusted setup. Open source. Tukar makes real-world money private and keeps it
...
camera. Just screen-record the live site and add a voiceover.

**Just open https://tukar-six.vercel.app**. There is nothing to install. Click
...
bar (a single click activates a real built-in testnet key, with no wallet to
install), or click **Connect wallet** to sign with your own Freighter.
Transactions are gated on this connection, so **Send** only enables once you're
connected. (Optional: open the
...
**Screen:** the landing hero, with the speeding orange light-streaks and the headline
...
with the headline *"USDC in. Private crossing. Local fiat out."* and the
...
> even advances the Merkle tree trustlessly, with a proof, so no admin can forge a
> root."
...
(the exact figure is live, because the pool reads Reflector on-chain).
**Say:**
> "On the receiving side the payment arrives still shielded. The amount is
> revealed only at the off-ramp edge, where it converts to local fiat. 500 USDC
> becomes about 8,560 pesos, at a rate read live from Reflector, Stellar's
> on-chain FX oracle, rather than a hardcoded number. Private through the middle,
> visible exactly where compliance needs it." *(Optional: click **Withdraw on-chain →** to
...
> USDC and nothing else: no keys, no blinding, no other payments. And the same
> proof is checked twice, once in the browser and once by the live Stellar
> verifier contract. That's compliant privacy."
...
**Say:**
> "Under the hood there's a hardened custody pool with real USDC, amount-bound
> deposits and withdrawals, double-spend protection, a fully trustless tree, and a
> real Powers-of-Tau trusted setup, all tested on-chain. Open source. Tukar makes
> real-world money private and keeps it compliant, on Stellar."
...
after the rejection**, so your very next **Send** is a normal, honest deposit and
you never have to untick it manually.

### Optional beat: bearer notes, receiving money on another phone (after Scene 3)
...
**Import →**, and the payment reappears, now tagged *imported*. Click **Withdraw
on-chain →**: it releases real tokens. Say: *"A completely different device, with
no shared account and no server, just imports the note and withdraws. The tree
reconstructs from chain anywhere, and the contract binds the payout to whoever
withdraws."* (Bonus, to show double-spend protection: **keep** the bearer string
...
**Request →**, and a string and a QR appear (copied to your clipboard). Say: *"The
receiver can also ask for money. Here's a request for 750 USDC as a string and a
QR, carrying just the amount and the payee address and no secrets."* Paste it into
...
fulfill it. Say: *"The sender loads the request and pays it as a normal shielded
