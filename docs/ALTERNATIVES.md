...
## 1. Launchtube → native fee-bump (CAP-15), DONE and live-verified
...
inner invoke, the app's relayer pays the fee, and deposits/withdrawals are gasless.
...
## 2. Mercury → durable contract state, already solved with no external dep
...
which has **no event-retention dependency** and is strictly more reliable than indexing
...
- **On-chain capability is real:** Stellar's `secp256r1_verify` host function (the
  ...
  **unit-testable in Rust with a known P-256 keypair**, so no authenticator is needed
  to prove the on-chain half works (the same way on-chain Poseidon was proven by unit
...
- **Why an ephemeral per-visitor key does NOT work here (corrected):** an earlier
  ...
  is not in the list, no membership proof exists for it, and the deposit is rejected both
...
that privacy-pool tier**, because the shielded transfer leg hides amount *and* counterparties,
...
updated without redeploying the pool. That is the same "swap the allow/deny registry"
property Confidential Tokens exposes, on the privacy-pool tier. The compliance *circuit* is
...
against SDF's public reference anchor** (`testanchor.stellar.org`) with real HTTP, real
signatures and no mocks (**5/5 steps pass**):
