...
- Observability: Sentry (@sentry/nextjs) and a structured JSON logger (lib/log.ts) are wired, the Vercel Marketplace integration is connected to the tukar project (org pugarhuda-r4, project sentry-erin-zebra), and a production deploy handshake uploaded source maps and registered a release. **But Sentry is not actually reporting.** `Sentry.init` is gated on `NEXT_PUBLIC_SENTRY_DSN`, that variable is not set on the deployment, and the production bundle contains no DSN, verified on 2026-09-11 by fetching the live page and finding zero ingest references. An earlier revision of this document claimed it was fully active, which was wrong. Setting the DSN is what turns it on; until then every log.error goes nowhere. Do not run the Sentry wizard, because the app is already instrumented.
