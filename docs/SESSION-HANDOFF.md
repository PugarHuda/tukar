...
- Observability deepened: Sentry (@sentry/nextjs, gated on NEXT_PUBLIC_SENTRY_DSN) + a structured JSON logger (lib/log.ts). Sentry is now fully ACTIVE: the Vercel Marketplace Sentry integration (Developer plan) is connected to the tukar project (org pugarhuda-r4, project sentry-erin-zebra), all seven env vars injected, and a production deploy handshake succeeded (source maps uploaded, release registered). The runtime SDK reports live; the next unhandled error lands in Sentry Issues with a de-minified stack. Do not run the Sentry wizard, because the app is already instrumented.
