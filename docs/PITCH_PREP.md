...
- **ZK is the product, not a buzzword.** 8 Circom / Groth16 / BN254 circuits, verified ON-CHAIN by deployed Soroban contracts. 15 contracts live on Stellar testnet (a core 8 of pool plus 7 verifiers, plus the reserves verifier, the per-corridor policy registry, two proof-of-reserves contracts, and three preview-track pools). Remove the proofs and Tukar does not exist.
...
- **Security done properly.** No double-spend (nullifiers are bound public inputs), a trustless Merkle tree with no admin backdoor, a critical non-canonical-field double-spend bug we found and fixed, and multi-party ceremony keys. 55/55 tests on the live pool, 333 across all 8 contract crates, 282 webapp tests, plus circuit soundness suites.
...
| If it is private, how does a regulator audit it? | Selective disclosure. The holder proves exactly one fact to a regulator (the exact amount, or that it is at or below a threshold, or a portfolio sum at or below a cap, or within a range), verified on-chain, and nothing else is revealed. Aggregate completeness is enforced on-chain by an audit-request registry so a holder cannot cherry-pick. Volunteer the bound: on the live testnet pool the auditor role is still the published demo key, so the gate is open there today even though the contract logic is real and tested. Repointing it is one admin call and a mainnet precondition. |
...
| What stops a double-spend? | Nullifiers are bound public inputs, so a spent note cannot be replayed. We found and fixed a critical non-canonical-field bug where re-encoded nullifiers could bypass the check. The tree is trustless (advances only via a merkleUpdate proof, no admin root backdoor). 55/55 tests on the live pool, 333 across all contract crates, plus circuit soundness suites. |
| Why Stellar? | Stellar exists to move real money across borders (anchors, SEP protocols, low fees), and Protocol 25/26 added native BN254 host functions that make on-chain Groth16 verification affordable (testnet runs Protocol 28 "Adapter" today, mainnet vote 2026-09-16). It is the natural home for a compliant confidential remittance rail. |
...
| (Technical) Is the ZK actually on-chain, and what part is not yet wired? | Real and on-chain: 8 Circom/Groth16/BN254 circuits, proofs generated in the browser and verified by 15 deployed Soroban contracts (a core 8 of pool plus 7 verifiers, plus the reserves verifier, the policy registry, two proof-of-reserves contracts and three preview-track pools), 55/55 pool tests and 333 across all contract crates, plus circuit-soundness suites. The Reflector FX read gates settlement on-chain, the Travel Rule exchange runs over OpenVASP TRP 3.2.1 with signatures verified on receipt, Circle CCTP V2 moves USDC both ways, and proof-of-reserves is exact. Not yet wired for production: the fiat anchor is protocol-integrated (real SEP-10/12/24/38 calls) but runs against SDF's reference testnet anchor and needs a licensed partner, per-corridor cap enforcement runs on a preview pool until a state migration moves it onto the live pool, TRISA needs an operator VASP registration, the public demo shares one testnet key, the phase-2 ceremony rounds were run to prove the process rather than on independent machines, and it is not professionally audited. We are explicit about that line. |
