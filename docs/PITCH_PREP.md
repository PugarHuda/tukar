...
| What stops a double-spend? | Nullifiers are bound public inputs, so a spent note cannot be replayed. We found and fixed a critical non-canonical-field bug where re-encoded nullifiers could bypass the check. The tree is trustless (advances only via a merkleUpdate proof, no admin root backdoor). 55/55 tests on the live pool, 317 across all contract crates, plus circuit soundness suites. |
