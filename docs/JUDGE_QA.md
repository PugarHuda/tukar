# Tukar, 20 judge questions with answers
...
a regulator. Bolting our compliance layer on is the year of work, not the afternoon.
...
**5. How are you different from Veil, OLIO, or any mixer?**
A mixer like Veil is private but cannot answer a regulator, so a licensed operator can't
touch it. A private payment link like OLIO moves crypto between users, not real fiat
across a border. We are the one that is private for the user and provable to a regulator,
on an actual remittance corridor with fiat at both edges. Private, compliant, and
cross-border at once. That combination has no off-the-shelf equivalent.

**6. Isn't this just Stellar Confidential Tokens with extra steps?**
...
**7. Privacy plus money movement is how Tornado Cash got sanctioned. Why aren't you a legal liability?**
...
**8. Is the zero-knowledge actually verified on-chain, or just in the browser?**
...
**9. What is honestly not wired yet, or still simplified?**
...
audited, so not for real money yet. Everything in the middle, the proofs, the deposit, the
verification, the oracle read, is real.

**10. How do you prevent double-spending?**
...
**11. Why Groth16 over BN254, and does Soroban really support that?**
...
**12. A trusted setup is a backdoor risk. What if it was compromised?**
...
**13. In the compliance proof, what stops me depositing as someone else?**
...
**14. Your anonymity set is tiny in a demo. Doesn't that break the privacy?**
...
**15. How does the oracle gate work, and what stops a manipulated price?**
...
**16. In an aggregate audit, can't the holder just leave out the payments they don't like?**
...
**17. Is proving on a phone actually realistic?**
...
**18. Why Stellar and not another chain?**
...
and Stellar is where those edges already live.

**19. Would a real remitter actually use this? What about onboarding?**
...
**20. What is real in the demo, and what would you do with the prize?**
...
demo key. With funding the first moves are a professional audit, a genuinely distributed
trusted-setup ceremony, and one licensed-anchor corridor live end to end, which is also the
path to a Stellar Community Fund build award.
